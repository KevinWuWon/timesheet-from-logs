...
_SENSITIVE_VALUE_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    (re.compile(r"\bvercel_blob_rw_[A-Za-z0-9_-]+\b"), "[REDACTED_VERCEL_BLOB_TOKEN]"),
    (re.compile(r"\bGOCSPX-[A-Za-z0-9_-]+\b"), "[REDACTED_GOOGLE_CLIENT_SECRET]"),
    (re.compile(r"\b\d+-[a-z0-9]+\.apps\.googleusercontent\.com\b"), "[REDACTED_GOOGLE_CLIENT_ID]"),
    (re.compile(r"\bsk_(?:test|live)_[A-Za-z0-9_-]+\b"), "[REDACTED_STRIPE_SECRET_KEY]"),
    (re.compile(r"\bsk-(?:proj|svcacct|ant)-[A-Za-z0-9_-]+\b"), "[REDACTED_API_KEY]"),
    (re.compile(r"\bAC[0-9a-fA-F]{32}\b"), "[REDACTED_TWILIO_ACCOUNT_SID]"),
)

_SENSITIVE_ASSIGNMENT_RE = re.compile(
    r"(?P<prefix>\b[A-Z0-9_]*(?:SECRET|TOKEN|KEY|CLIENT_ID|CLIENT_SECRET|ACCOUNT_SID)[A-Z0-9_]*\s*[=:]\s*)"
    r"(?P<quote>[\"']?)"
    r"(?P<value>[^\s,\"'<>]+)"
    r"(?P=quote)",
    re.IGNORECASE,
...
def _redact_sensitive_text(text: str) -> str:
    """Redact secret-looking values before they enter summaries or HTML reports."""
    redacted = text
    ...
    return _SENSITIVE_ASSIGNMENT_RE.sub(repl, redacted)
...
def _user_text(payload: dict) -> str:
    ...
    return _redact_sensitive_text(_strip_system_content(raw))
...
def main() -> int:
    ...
    def out_path_for(y: int, w: int) -> Path:
        ...
        return Path.cwd() / "outputs" / f"timeline-codex-{y}-W{w:02d}.html"
